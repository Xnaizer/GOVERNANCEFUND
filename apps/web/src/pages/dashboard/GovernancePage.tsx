...
import { Check, ChevronsUpDown, UserCheck, Briefcase, BarChart3, Info, Gavel, Users } from "lucide-react";
...
function RowAvatar({ u }: { u: AdminUser }) {
  return (
    <Avatar className="h-9 w-9 shrink-0">
      {u.profilePictureURL && <AvatarImage src={u.profilePictureURL} alt={u.name ?? u.username} />}
      <AvatarFallback className="text-[10px]">{initials(u.name ?? u.username)}</AvatarFallback>
    </Avatar>
  );
}

/* ── Baris user USER: verifikasi (via detail) lalu grant PIC ── */
function PicGrantRow({ u, onDetail }: { u: AdminUser; onDetail: (id: string) => void }) {
    ...
    <div className="flex flex-wrap items-center gap-2 border-b border-black/5 py-2.5 last:border-0">
      ...
      <div className="min-w-0 flex-1 text-sm">
        ...
        {wallet ? (
          ...
          <Badge variant="secondary" className="rounded-sm">tanpa wallet</Badge>
        )}
        {u.isVerified ? (
      ...
      {/* Verifikasi HARUS lewat detail identitas (butuh field lengkap) */}
      <Button size="sm" variant={u.isVerified ? "ghost" : "secondary"} onClick={() => onDetail(u.id)}>
      ...
      {u.isVerified && !wallet && (
        <Badge variant="warning" className="rounded-sm">user belum bind wallet</Badge>
      )}
    </div>
  );
}
...
function PicActiveRow({ u, onDetail }: { u: AdminUser; onDetail: (id: string) => void }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 border-b border-black/5 py-2.5 last:border-0">
      ...
      <Button size="sm" variant="ghost" onClick={() => onDetail(u.id)}>Lihat detail</Button>
      <ConfirmButton
          ...
          {selected ? (
            <span className="flex min-w-0 items-center gap-2">
              <Avatar className="h-6 w-6 shrink-0">
                {selected.profilePictureURL && <AvatarImage src={selected.profilePictureURL} alt={selected.name ?? selected.username} />}
                <AvatarFallback className="text-[9px]">{initials(selected.name ?? selected.username)}</AvatarFallback>
              </Avatar>
              <span className="truncate">{selected.name ?? selected.username}</span>
            </span>
          ) : (
...
export function GovernancePage() {
  ...
  const votes = useQuery({ queryKey: ["role-votes", "gov"], queryFn: () => fetchRoleVotes({ limit: 50 }) });
  ...
  const usersToGrant = allUsers.filter((u) => u.role === "USER" && matchUser(u)); // prospektif PIC (khusus USER)
  ...
  // Daftar "Semua Pengguna" (tab BFT) — bantu admin cek detail kandidat sebelum vote.
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Tabs defaultValue="pic" className="min-w-0">
          ...
          <div className="flex flex-col gap-6 pt-2">
            ...
            <Card className="rounded-2xl border-black/5 shadow-none">
              ...
              <CardContent>
                ...
                >
                  {usersToGrant.map((u) => (
                    <PicGrantRow key={u.id} u={u} onDetail={setDetailUserId} />
                  ))}
                </QueryState>
              </CardContent>
            </Card>
            ...
            <Card className="rounded-2xl border-black/5 shadow-none">
              ...
              <CardContent>
                {activePics.length === 0 ? (
                  <EmptyState icon={<Briefcase />} title="Belum ada PIC aktif" description="PIC yang di-grant akan muncul di sini." />
                ) : (
                  activePics.map((u) => <PicActiveRow key={u.id} u={u} onDetail={setDetailUserId} />)
                )}
              </CardContent>
            </Card>
          </div>
        ...
        <TabsContent value="vote">
          <div className="flex flex-col gap-6 pt-2">
            ...
            {/* Semua Pengguna — cek detail kandidat sebelum vote */}
            <Card className="rounded-2xl border-black/5 shadow-none">
              <CardHeader className="flex-row items-center gap-2 space-y-0 font-display font-semibold tracking-tight">
                <Users className="h-4 w-4 text-brand-blue" />
                Semua Pengguna
                <Badge variant="secondary" className="ml-auto rounded-sm">{allUsers.length}</Badge>
              </CardHeader>
                ...
                >
                  <div className="flex max-h-104 flex-col overflow-y-auto pr-1">
                    {allUsersFiltered.map((u) => (
                      <div key={u.id} className="flex flex-wrap items-center gap-2 border-b border-black/5 py-2.5 last:border-0">
                        <RowAvatar u={u} />
                        ...
                        <Button size="sm" variant="ghost" onClick={() => setDetailUserId(u.id)}>Detail</Button>
                      </div>
                    ))}
                  </div>
                </QueryState>
        ...
        {/* ── TAB 3: Voting peran yang saya lakukan ── */}
      ...
      {/* Panel kanan — ringkasan governance */}
      <aside className="flex flex-col gap-4">
        ...
        <div className="rounded-2xl border border-black/5 bg-muted/30 p-5">
          ...
          <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
            Grant PIC langsung oleh 1 admin (khusus USER terverifikasi). Perubahan peran struktural
            (Validator/Auditor/Admin) butuh voting BFT ⌊2N/3⌋+1 dan tercatat on-chain.
          </p>
        </div>
      </aside>
      </div>
