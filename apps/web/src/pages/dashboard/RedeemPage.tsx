...
export function RedeemPage() {
  ...
  return (
    <>
      ...
      <div className="flex max-w-lg flex-col gap-6">
        ...
        <Card className="rounded-2xl border-black/5 shadow-none">
          <CardContent className="p-5">
            <p className="text-[11px] font-medium uppercase tracking-[0.14em] text-muted-foreground">Saldo e-IDR</p>
            <p className="mt-1 font-display text-3xl font-semibold tracking-tight text-brand-blue">{formatIDR(balance.toString())}</p>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-2xl border-black/5 shadow-none">
          <CardHeader className="flex-row items-center justify-between space-y-0 font-display font-semibold tracking-tight">
            ...
            <Badge variant="secondary" className="rounded-sm">escrow · non-transferable</Badge>
          </CardHeader>
        ...
        <Card className="rounded-2xl border-black/5 shadow-none">
          <CardHeader className="font-display font-semibold tracking-tight">Permintaan Penukaran Saya</CardHeader>
          <CardContent className="flex flex-col gap-2">
            {loadingRequests
              ...
              : requests.length === 0
                ...
                : requests.map((r) => (
                  <div key={r.id.toString()} className="flex flex-wrap items-center gap-2 border-b border-black/5 py-2.5 text-sm last:border-0">
                    ...
                    <Badge variant={STATUS_VARIANT[r.status]} className="rounded-sm">{STATUS_LABEL[r.status]}</Badge>
      ...
      {/* Panel pemandangan (kanan) — agar tak sepi + ringkasan alur penukaran */}
      <aside className="hidden lg:block">
        <div className="sticky top-6 overflow-hidden rounded-2xl border border-black/5">
          ...
          <div className="flex flex-col gap-3 bg-muted/30 p-5 text-sm">
            {[
              { n: "1", t: "Ajukan penukaran", d: "Token dititipkan ke gateway (escrow), belum dibakar." },
              { n: "2", t: "Operator bank verifikasi", d: "Bank membakar token & mencairkan Rupiah fisik." },
              { n: "3", t: "Tarik kembali (opsional)", d: `Bila tak diproses, tarik escrow setelah ${timeoutDays} hari.` },
            ].map((s) => (
              <div key={s.n} className="flex gap-3">
                ...
                <span>
                  <span className="block font-medium text-foreground">{s.t}</span>
            ...
            <p className="mt-1 rounded-lg bg-brand-blue/5 px-3 py-2 text-xs text-muted-foreground">
              1 Rupiah = 1 token eIDR · token non-transferable (hanya bisa dibakar di gateway).
            </p>
          </div>
        </div>
      </aside>
