...
import { Camera, Pencil, Wallet, ShieldCheck, Award, BadgeCheck, IdCard } from "lucide-react";
...
export function ProfilePage() {
  ...
  const repTone = me.reputationScore < 35 ? "text-destructive" : me.reputationScore < 50 ? "text-amber-600" : "text-emerald-600";

  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-6">
      <PageHeader eyebrow="Akun" title="Profil Saya" gradient subtitle="Kelola identitas, wallet, dan foto profil Anda." />
        ...
        <div className="relative">
          ...
          <div className="absolute -bottom-11 left-6">
            <div className="relative">
              ...
              >
                {avatarUp.isPending ? <Spinner size={14} className="text-current" /> : <Pencil className="h-3.5 w-3.5" />}
        ...
        <div className="flex flex-col gap-4 px-6 pb-6 pt-14 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h2 className="font-display text-2xl font-semibold tracking-tight">{me.name ?? me.username}</h2>
              <Badge className="rounded-full">{me.role}</Badge>
              {me.isVerified && (
                <Badge variant="success" className="gap-1 rounded-full"><BadgeCheck className="h-3 w-3" /> Verified</Badge>
              )}
            </div>
          ...
          <div className="flex items-center gap-2 rounded-2xl border border-black/5 bg-muted/40 px-4 py-2.5">
            ...
            <div>
              <p className={`font-display text-xl font-semibold leading-none ${repTone}`}>{me.reputationScore}</p>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="flex flex-col gap-6">
            ...
            <CardContent className="flex flex-col gap-3">
              {me.walletAddress ? (
                <div className="flex items-center gap-2 rounded-xl border border-emerald-500/20 bg-emerald-500/5 px-3 py-2.5">
                  ...
                  <span className="font-mono text-sm">{formatShortenAddress(me.walletAddress)}</span>
                  <Badge variant="success" className="ml-auto rounded-full">terhubung</Badge>
                </div>
              ) : (
                <>
                  <p className="text-sm text-muted-foreground">
                    Hubungkan lalu tanda tangani nonce untuk mengikat wallet ke akun (sekali, sebelum punya role).
                  </p>
          ...
          <Card className="rounded-2xl border-black/5 shadow-none">
            <CardHeader className="font-display font-semibold tracking-tight">Status Akun</CardHeader>
            <CardContent className="flex flex-col divide-y divide-black/5 text-sm">
              <div className="flex items-center justify-between py-2.5">
                ...
                <Badge variant={me.isActive ? "success" : "warning"} className="rounded-full">{me.isActive ? "Ya" : "Belum"}</Badge>
              </div>
              <div className="flex items-center justify-between py-2.5">
                <span className="text-muted-foreground">Identitas diverifikasi</span>
                <Badge variant={me.isVerified ? "success" : "warning"} className="rounded-full">{me.isVerified ? "Ya" : "Belum"}</Badge>
              </div>
              <div className="flex items-center justify-between py-2.5">
