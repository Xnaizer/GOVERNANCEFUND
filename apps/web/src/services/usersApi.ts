...
export interface AdminUserDetail {
  id: string; username: string; email: string; role: Role;
  isActive: boolean; isVerified: boolean; walletAddress: string | null;
  name: string | null; nik: string | null; nip: string | null;
  institution: string | null; position: string | null;
  birthPlace: string | null; birthDate: string | null;
  address: string | null; phone: string | null; nationality: string | null;
  reputationScore: number; profilePictureURL: string | null; profileBannerURL: string | null;
  createdAt: string; programsCount: number;
  missingFields: string[]; isProfileComplete: boolean;
}

// Detail identitas lengkap untuk ADMIN (privat) — dipakai saat verifikasi.
