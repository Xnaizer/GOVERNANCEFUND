...
export async function fetchRoleVotes(params: { page?: number; limit?: number } = {}) {
  const res = await api.get<Envelope<RoleVoteRow[]>>("/public/votes", { params: { limit: 12, ...params } });
...
export async function fetchRoleVote(voteId: number) {
  const res = await api.get<Envelope<RoleVoteDetail>>(`/public/votes/${voteId}`);
...
export async function fetchUnfreezeVotes(params: { page?: number; limit?: number } = {}) {
  const res = await api.get<Envelope<UnfreezeVoteRow[]>>("/public/unfreeze-votes", { params: { limit: 12, ...params } });
  return { rows: res.data.data, pagination: res.data.meta.pagination };
}
