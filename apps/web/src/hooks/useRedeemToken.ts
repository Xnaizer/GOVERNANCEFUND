...
export function useRedeemToken() {
  ...
  const timeoutQ = useReadContract({
    ...gatewayContract,
    chainId: CHAIN_ID,
    functionName: "RECLAIM_TIMEOUT",
  });
  const reclaimTimeout = (timeoutQ.data as bigint | undefined) ?? 604800n;
      ...
      const infos = await Promise.all(
        ids.map(async (id) => {
          ...
          return {
            ...
            status,
            canReclaim:
              status === "PENDING" && now >= r.createdAt + reclaimTimeout,
    ...
    try {
      ...
      if (allowance < amount) {
        ...
        const ha = await writeContractAsync({
          ...
          args: [gatewayContract.address, amount],
        });
  ...
  const busy =
    state === "approving" || state === "requesting" || state === "reclaiming";
  return {
    requestRedemption,
    reclaim,
    balance,
