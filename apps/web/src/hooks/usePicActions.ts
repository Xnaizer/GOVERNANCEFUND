...
export function useWithdraw(programId: number) {
  ...
  const withdraw = (amount: string, recipientName: string, description: string) =>
    tx.execute({
...
export function useFinalizeMilestone(programId: number) {
  const tx = useTxThenSync({
    waitForSync: async () => (await getProgramDetailAuthed(programId)).status !== "DRAWABLE", // → MILESTONE_ACHIEVED / COMPLETED
  });
  const finalize = () =>
    tx.execute({ ...governanceContract, chainId: CHAIN_ID, functionName: "finalizeMilestone", args: [BigInt(programId)] });
  return { ...tx, finalize };
}
...
export function useProposeAppeal(programId: number) {
  ...
  const propose = () =>
    tx.execute({ ...governanceContract, chainId: CHAIN_ID, functionName: "proposeUnfreezeAppeal", args: [BigInt(programId)] });
  return { ...tx, propose };
}
