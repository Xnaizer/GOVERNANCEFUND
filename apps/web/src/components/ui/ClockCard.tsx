...
function monthGrid(year: number, month: number): (number | null)[] {
  const first = new Date(year, month, 1).getDay(); // 0=Min
  ...
  return (
    <Card>
      <CardContent className="flex flex-col gap-3 p-4">
        ...
        <div>
          <p className="mb-1 text-xs font-semibold uppercase text-muted-foreground">{monthName}</p>
          <div className="grid grid-cols-7 gap-0.5 text-center text-[10px]">
            {DOW.map((d) => <span key={d} className="py-0.5 font-medium text-muted-foreground/70">{d}</span>)}
            {cells.map((c, i) => (
              <span
                key={i}
                className={cn(
                  "rounded py-0.5",
                  c === today ? "bg-brand-blue font-bold text-white" : c ? "text-foreground/80" : "",
                )}
              >
                {c ?? ""}
              </span>
