...
function Wordmark({ className }: { className?: string }) {
  return (
    <Link to="/" className={`flex w-fit flex-col leading-[0.95] ${className ?? ""}`}>
      <span className="font-display text-sm font-bold tracking-[0.2em]">GOVERNANCE</span>
...
export function AuthLayout({
  ...
  return (
    <div className="grid min-h-screen lg:grid-cols-2">
      {/* Kiri: bg vektor 3D + sapaan simpel */}
      <div className="relative hidden overflow-hidden bg-[#0b1220] text-white lg:flex lg:flex-col lg:justify-between lg:p-12">
        <Aurora className="opacity-70" />
        {/* Bola gradient 3D melayang */}
        <div aria-hidden className="pointer-events-none absolute inset-0">
          <div
            className="absolute right-[-6%] top-[10%] h-52 w-52 rounded-full"
            style={{ background: "radial-gradient(circle at 32% 26%, #bff6ea, #4899ea 46%, #21407c 100%)", boxShadow: "0 40px 80px -20px rgba(72,153,234,0.55)" }}
          />
          <div
            className="absolute bottom-[-8%] left-[-8%] h-64 w-64 rounded-full"
            style={{ background: "radial-gradient(circle at 30% 24%, #dcccff, #8b5cf6 48%, #3a1f78 100%)", boxShadow: "0 40px 80px -20px rgba(139,92,246,0.5)" }}
          />
          <div
            className="absolute bottom-[24%] right-[22%] h-24 w-24 rounded-full"
            style={{ background: "radial-gradient(circle at 32% 26%, #ccfff2, #67f3ce 50%, #128a72 100%)", boxShadow: "0 24px 50px -16px rgba(103,243,206,0.55)" }}
          />
        </div>
        <Grain className="opacity-40" />

        {/* Navbar kiri: wordmark = balik beranda */}
        <Wordmark className="relative" />

        {/* Sapaan simpel */}
        <div className="relative">
          <h2 className="max-w-md font-display text-4xl font-semibold leading-[1.1] tracking-tight">
            {greeting?.title ?? <>Dana publik untuk <span className="text-gradient">setiap warga.</span></>}
          </h2>
          {greeting?.text && <p className="mt-4 max-w-sm text-white/60">{greeting.text}</p>}
        </div>

        <p className="relative text-xs text-white/50">Base Sepolia · Proyek akademik</p>
      </div>

      {/* Kanan: navbar + form */}
      <div className="relative flex flex-col px-5 py-5 sm:px-12 sm:py-6">
        {/* Aksen aurora untuk mobile/tablet (panel kiri disembunyikan) */}
        <div aria-hidden className="pointer-events-none absolute inset-x-0 top-0 h-64 overflow-hidden lg:hidden">
