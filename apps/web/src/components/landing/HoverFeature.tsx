  ...
  return (
    <section data-nav-theme="light" className="bg-background py-24 sm:py-32">
      <div className="mx-auto grid max-w-6xl items-center gap-12 px-6 lg:grid-cols-2 lg:gap-20">
        {/* Kiri: info + 3 item hoverable */}
        <div>
          <Reveal>
            <span className="text-xs font-medium uppercase tracking-[0.25em] text-brand-blue">Satu sistem, banyak mata</span>
            <h2 className="mt-3 font-display text-2xl font-semibold leading-[1.1] tracking-tight sm:text-4xl">
              Transparansi bukan sekadar janji. <span className="text-gradient">Ini bisa dibuktikan.</span>
            </h2>
          </Reveal>
              ...
              return (
                ...
                >
                  ...
                  <span
                    ...
                    style={{ backgroundColor: it.color, height: on ? "70%" : "0%" }}
                  />
                  <span
                    className={cn("text-xs font-semibold uppercase tracking-[0.2em] transition-colors", !on && "text-muted-foreground")}
                    style={on ? { color: it.color } : undefined}
                  >
                    {it.tag}
                  </span>
                  <h3 className={cn("mt-2 font-display text-lg font-semibold tracking-tight transition-opacity sm:text-xl", !on && "lg:opacity-55")}>
                    {it.title}
                  </h3>
                  <p className={cn("mt-1.5 max-w-md text-pretty text-sm leading-relaxed text-muted-foreground transition-all", on ? "opacity-100" : "opacity-100 lg:opacity-0")}>
                    {it.body}
                  </p>
                </button>
              );
        ...
        {/* Kanan: foto berganti (crossfade) + glow warna + chip mengambang */}
        <Reveal className="relative">
          ...
          <div className="relative aspect-4/5 overflow-hidden rounded-4xl shadow-glow">
            {/* Ganti gambar langsung, tanpa animasi */}
            <img src={item.img} alt={item.title} className="absolute inset-0 h-full w-full object-cover" />
            {/* tint warna aktif + gradient bawah */}
            <div className="absolute inset-0 mix-blend-soft-light" style={{ backgroundColor: item.color, opacity: 0.25 }} />
