...
export function ProgramFlow() {
  ...
  const fill = useSpring(scrollYProgress, { stiffness: 90, damping: 30, mass: 0.5 });

  return (
    // White space di ujung → blok gelap membulat "di dalam" latar terang
    <section className="bg-background py-6 sm:py-10">
      ...
      >
        {/* Ambient glow */}
        <Glow color="rgba(72,153,234,0.16)" size="55%" className="left-[-8%] top-[8%] h-[40rem] w-[40rem]" />
        <Glow color="rgba(103,243,206,0.12)" size="55%" className="right-[-8%] bottom-[10%] h-[36rem] w-[36rem]" />

        {/* Header rata kiri + pill */}
        <div className="relative max-w-2xl">
          <Reveal>
            ...
            <p className="mt-4 text-pretty text-sm text-white/60 sm:text-base">
              Lima langkah, semuanya terekam. Tiap simpul adalah pemeriksaan yang tak bisa dilewati.
            </p>
          </Reveal>
        </div>
        ...
        <div ref={track} className="relative mt-16">
          {/* Garis tengah + isian saat scroll */}
            ...
            return (
              <div key={s.title} className="relative py-8 lg:py-14">
                ...
                <div className="grid items-center gap-8 lg:grid-cols-2 lg:gap-12">
                  {/* Info kiri — rata kiri, posisi dekat garis tengah */}
                  <Reveal className="order-2 lg:order-1">
                    <div className="mx-auto max-w-sm lg:mr-0 lg:ml-auto">
                      <h3 className="font-display text-xl font-semibold tracking-tight sm:text-3xl">
                        <span style={{ color: s.color }}>{first}</span> {rest.join(" ")}
                      </h3>
                      <p className="mt-3 text-pretty text-sm leading-relaxed text-white/60">{s.body}</p>
                    </div>
                  </Reveal>

                  {/* Gambar kanan — tanpa border/shadow, glow warna di belakang */}
                  <Reveal className="relative order-1 lg:order-2 lg:pl-16" delay={0.08}>
        ...
        {/* Konektor garis + badge + CTA minimalist */}
        <div className="relative mx-auto max-w-2xl pt-24 text-center">
          {/* Garis vertikal menyambung SAMPAI ke badge */}
          ...
          {/* Badge logo — menempel di atas card, garis berakhir di baliknya */}
          ...
          {/* Card CTA bersih & terpusat */}
          <div className="relative overflow-hidden rounded-4xl border border-white/10 bg-white/4 px-8 py-14 sm:px-14">
            {/* Aurora bg berwarna */}
            <Glow color="rgba(103,243,206,0.22)" size="60%" className="left-[12%] top-[-15%] h-64 w-64" />
            <Glow color="rgba(72,153,234,0.26)" size="60%" className="left-1/2 top-[-25%] h-80 w-80 -translate-x-1/2" />
            <Glow color="rgba(192,132,252,0.22)" size="60%" className="right-[12%] top-[-10%] h-64 w-64" />
            ...
            <p className="relative mx-auto mt-3 max-w-md text-pretty text-sm text-white/60 sm:text-base">
              Daftarkan institusi Anda dan mulai kelola anggaran yang tak bisa dicurangi — atau telusuri dulu
              lewat Explorer publik.
            </p>
